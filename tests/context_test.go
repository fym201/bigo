	...
	Convey("Do advanced encapsulation operations", t, func() {
		...
		Convey("Get request body", func() {
			...
			for i := 1; i <= 3; i++ {
				...
				req, err := http.NewRequest("GET", "/body"+utl.ToStr(i), nil)
		...
		Convey("Parse from and query", func() {
			m.Get("/query", func(ctx *Context) string {
				...
				buf.WriteString(utl.ToStr(ctx.QueryInt("int")) + " ")
				buf.WriteString(utl.ToStr(ctx.QueryInt64("int64")) + " ")
				buf.WriteString(utl.ToStr(ctx.QueryFloat64("float64")) + " ")
				return buf.String()
			})
		...
		Convey("URL parameter", func() {
			m.Get("/:name/:int/:int64/:float64", func(ctx *Context) string {
				...
				buf.WriteString(utl.ToStr(ctx.ParamsInt(":int")) + " ")
				buf.WriteString(utl.ToStr(ctx.ParamsInt64(":int64")) + " ")
				buf.WriteString(utl.ToStr(ctx.ParamsFloat64(":float64")) + " ")
				return buf.String()
			})
