...
func Test_Static(t *testing.T) {
	Convey("Serve static files", t, func() {
		...
		req, err := http.NewRequest("GET", "http://localhost:4000/bigo.go", nil)
		So(err, ShouldBeNil)
	...
	Convey("Serve static files with head", t, func() {
		...
		resp := httptest.NewRecorder()
		resp.Body = new(bytes.Buffer)
		req, err := http.NewRequest("HEAD", "http://localhost:4000/bigo.go", nil)
		So(err, ShouldBeNil)
		m.ServeHTTP(resp, req)
		So(resp.Code, ShouldEqual, http.StatusOK)
	...
	Convey("Serve static files as post", t, func() {
		...
		resp := httptest.NewRecorder()
		req, err := http.NewRequest("POST", "http://localhost:4000/bigo.go", nil)
		So(err, ShouldBeNil)
		m.ServeHTTP(resp, req)
...
func Test_Static_Options(t *testing.T) {
	Convey("Serve static files with options logging", t, func() {
		...
		So(resp.Code, ShouldEqual, http.StatusOK)
		So(buf.String(), ShouldEqual, "[Bigo] [Static] Serving /bigo.go\n")
	...
	Convey("Serve static files with options serve index", t, func() {
		var buf bytes.Buffer
		m := NewWithLogger(&buf)
		opt := StaticOptions{IndexFile: "bigo.go"}
		m.Use(Static(currentRoot, opt))
	...
	Convey("Serve static files with options prefix", t, func() {
		...
		resp := httptest.NewRecorder()
		req, err := http.NewRequest("GET", "http://localhost:4000/public/bigo.go", nil)
		So(err, ShouldBeNil)
		m.ServeHTTP(resp, req)
...
func Test_Statics(t *testing.T) {
	Convey("Serve multiple static routers", t, func() {
		...
		Convey("Serve normally", func() {
			...
			So(resp.Code, ShouldEqual, http.StatusOK)
			So(buf.String(), ShouldEndWith, "[Bigo] [Static] Serving /inject/inject.go\n")
		})
	})
}
